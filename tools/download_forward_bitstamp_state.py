...
def collect_raw(
    ...
    while cursor >= start_open:
        ...
        if not batch:
            break
        ts = [int(row["timestamp"]) for row in batch]
        for row in batch:
            t = int(row["timestamp"])
...
def collect_forward_state(
    *,
    ...
    end_day: date,
    mode: str,
    raw_collector: Callable = collect_raw,
) -> dict:
    ...
    if cutoff <= FREEZE:
        ...
        (state_dir / "DATA_GATE.json").write_text(
    ...
    for label, step in STEPS.items():
        ...
        frame, requests = raw_collector("btcusd", step, start_open, end_open)
        manifest.extend(requests)

        if frame.empty:
            eligible = frame
        else:
            frame["availability_time"] = pd.to_datetime(
                frame["availability_time"], utc=True
            )
            eligible = frame[
                (frame.availability_time > FREEZE)
                & (frame.availability_time <= cutoff)
            ].copy()
        ...
        if len(eligible):
            duplicate_timestamps = int(eligible.availability_time.duplicated().sum())
            monotonic = bool(eligible.availability_time.is_monotonic_increasing)
            envelope_failures = (
                (eligible.high < eligible[["open", "close", "low"]].max(axis=1))
                | (eligible.low > eligible[["open", "close", "high"]].min(axis=1))
            )
            nonpositive = (
                eligible[["open", "high", "low", "close"]] <= 0
            ).any(axis=1)
            out_of_range = (
                (eligible.availability_time <= FREEZE)
            ...
            monotonic = True
            envelope_failures = pd.Series(dtype=bool)
            nonpositive = pd.Series(dtype=bool)
        ...
        audits.append(
            {
                ...
                "eligible_start_availability": (
                    str(eligible.availability_time.iloc[0]) if len(eligible) else None
                ),
                "eligible_end_availability": (
                    str(eligible.availability_time.iloc[-1]) if len(eligible) else None
                ),
                "duplicate_timestamps": duplicate_timestamps,
                "monotonic": monotonic,
                "ohlc_envelope_failures": (
                    int(envelope_failures.sum()) if len(envelope_failures) else 0
                ),
                "nonpositive_price_rows": (
                    int(nonpositive.sum()) if len(nonpositive) else 0
                ),
                "availability_out_of_range": (
                    int(out_of_range.sum()) if len(out_of_range) else 0
                ),
    ...
    hard_failures = audit[audit.status == "FAIL"] if len(audit) else audit
    overall = (
        "FAIL"
        if len(hard_failures)
        else ("WAITING_FIRST_COMPLETED_STATE_BAR" if waiting else "PASS")
    )
    gate = {
        "status": overall,
        "classification": "POST_FREEZE_BITSTAMP_STATE_ONLY_DATA",
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=("monitor", "full"), default="monitor")
